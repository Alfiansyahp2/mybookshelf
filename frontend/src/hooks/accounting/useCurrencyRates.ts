...
export function useHistoricalCurrencyRates(
    fromCurrency: string,
    toCurrency?: string,
    date?: string,
) {
    return useQuery({
        ...
        queryFn: () =>
            currencyApi.getHistoricalRates(fromCurrency, toCurrency, date),
        enabled: !!fromCurrency,
...
export function useValidateCurrency() {
    return useMutation({
        mutationFn: (currencyCode: string) =>
            currencyApi.validateCurrency(currencyCode),
    });
}
...
export function useDeactivateOldCurrencyRates() {
    ...
    return useMutation({
        mutationFn: (daysToKeep?: number) =>
            currencyApi.deactivateOldRates(daysToKeep),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["currency-rates"] });
        },
    });
