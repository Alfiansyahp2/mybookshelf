...
export async function debugBooks() {
    ...
    try {
        ...
        console.log(
            `📝 Books with personal notes: ${booksWithNotes.length}/${books.length}`,
        );
        console.log(
            "📝 Books with notes:",
            booksWithNotes.map((b: any) => b.title),
        );

        console.log(
            `⭐ Books with ratings: ${booksWithRating.length}/${books.length}`,
        );
        console.log(
            "⭐ Books with ratings:",
            booksWithRating.map(
                (b: any) => `${b.title} (${b.personalRating}★)`,
            ),
        );
