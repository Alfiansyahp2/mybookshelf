...
export const expensesApi = {
    ...
    // Get single expense
    getExpense: (id: string) => apiClient.get(`/v1/accounting/expenses/${id}`),
    ...
    uploadReceipt: (
        id: string,
        receiptData: { data: string; mime_type?: string; filename?: string },
    ...
    convertCurrency: (id: string, targetCurrency: string) =>
        apiClient.post(`/v1/accounting/expenses/${id}/convert-currency`, {
...
export const expenseCategoriesApi = {
    // Get all categories
    getCategories: () => apiClient.get("/v1/accounting/categories"),
...
export const budgetsApi = {
    ...
    // Get single budget
    getBudget: (id: string) => apiClient.get(`/v1/accounting/budgets/${id}`),
    ...
    // Check budget alerts
    checkAlerts: () => apiClient.get("/v1/accounting/budgets/check-alerts"),

    // Get budget summary
    getSummary: () => apiClient.get("/v1/accounting/budgets/summary"),

    // Get top spending categories
    getTopSpendingCategories: (params?: {
        start_date?: string;
        end_date?: string;
        limit?: number;
    }) =>
...
export const currencyApi = {
    ...
    getRates: () => apiClient.get("/v1/accounting/currency/rates"),

    // Get specific rate
    getRate: (fromCurrency: string, toCurrency?: string) =>
        apiClient.get("/v1/accounting/currency/rates/current", {
            from_currency: fromCurrency,
            to_currency: toCurrency || "IDR",
        }),
    ...
    convert: (amount: number, fromCurrency: string, toCurrency?: string) =>
        apiClient.post("/v1/accounting/currency/convert", {
            amount,
            from_currency: fromCurrency,
            to_currency: toCurrency || "IDR",
        }),
    ...
    // Sync live rates
    syncRates: () => apiClient.post("/v1/accounting/currency/rates/sync"),
    ...
    getHistoricalRates: (
        ...
        apiClient.get("/v1/accounting/currency/rates/historical", {
            from_currency: fromCurrency,
            to_currency: toCurrency || "IDR",
            date,
        }),
    ...
    validateCurrency: (currencyCode: string) =>
        apiClient.get("/v1/accounting/currency/validate", {
            currency_code: currencyCode,
        }),

    // Get currency info
    getCurrencyInfo: (currencyCode: string) =>
        apiClient.get("/v1/accounting/currency/info", {
            currency_code: currencyCode,
        }),

    // Deactivate old rates
    deactivateOldRates: (daysToKeep?: number) =>
        apiClient.post("/v1/accounting/currency/rates/deactivate-old", {
...
export const reportsApi = {
    ...
    getPaymentMethodsAnalysis: (params?: {
        start_date?: string;
        end_date?: string;
    ...
    getMonthlyComparison: (months?: number) =>
        apiClient.get("/v1/accounting/reports/monthly-comparison", {
            months: months || 6,
        }),
