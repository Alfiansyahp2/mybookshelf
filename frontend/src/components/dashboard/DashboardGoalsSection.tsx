        ...
        <Card>
          <div style={{ padding: '16px 18px', background: `linear-gradient(135deg,${BRAND.darkBrown} 0%,#6b4528 100%)`, borderRadius: '16px 16px 0 0' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <h3 style={{ margin: 0, fontSize: 13, fontWeight: 700, color: 'rgba(255,210,140,0.9)', fontFamily: "'Georgia',serif", display: 'flex', alignItems: 'center', gap: 6 }}>
                <Target size={14} /> {t('dashboard.goals.target', { year: stats.currentYear })}
              </h3>
              <span style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,251,245,0.7)' }}>{t('dashboard.goals.achieved', { pct: Math.round(goalPct) })}</span>
            </div>
          ...
          <div style={{ padding: '12px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(139,99,56,0.03)' }}>
            <div>
              ...
              <span style={{ fontSize: 11, color: BRAND.walnut, marginLeft: 6, fontWeight: 600 }}>/ {stats.target} {t('dashboard.goals.books')}</span>
            </div>
            <div style={{ fontSize: 10, color: BRAND.walnut, opacity: 0.7, textAlign: 'right', lineHeight: 1.3 }}>
              {t('dashboard.goals.remaining', { count: Math.max(0, stats.target - stats.finishedThisYear) })}
            </div>
          </div>
        </Card>
      </motion.div>
      ...
      <motion.div {...fadeUp(0.45)}>
        <Card style={{ padding: '16px 18px' }}>
          <h3 style={{ margin: '0 0 12px', fontSize: 13, fontWeight: 700, color: BRAND.darkBrown, fontFamily: "'Georgia',serif", display: 'flex', alignItems: 'center', gap: 6 }}>
            <Award size={14} color={BRAND.walnut} /> {t('dashboard.goals.quick_stats')}
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              { label: t('dashboard.goals.fav_genre'), val: stats.topGenre, icon: '📚' },
              { label: t('dashboard.goals.avg_rating'), val: stats.avgRating > 0 ? `⭐ ${stats.avgRating}` : '—', icon: '⭐' },
              { label: t('dashboard.goals.total_shelves'), val: `${shelvesLength} ${t('dashboard.goals.shelves')}`, icon: '🪵' },
              { label: t('dashboard.goals.wishlist'), val: `${stats.wishlist} ${t('dashboard.goals.books')}`, icon: '🛒' },
            ].map(s => (
