...
interface BookshelfProps {
    ...
    isEditMode?: boolean;
    onSaveLayout?: (
        layoutData: { id: string; order: number; span: number }[],
    ) => void;
...
const FRAME = {
    ...
    sideBar:
        "linear-gradient(to right, #7a5428 0%, #9a6e40 40%, #8a6030 70%, #6a4618 100%)",
    basePlinth:
        "linear-gradient(180deg, #8a6030 0%, #6a4820 50%, #543814 100%)",
};
...
export default function Bookshelf({
    ...
    const distributeBooksAcrossShelves = (): Map<string, Book[]> => {
        ...
        sorted.forEach((book) => {
            if (book.shelfId && dist.has(book.shelfId))
                dist.get(book.shelfId)!.push(book);
        });
        return dist;
    };
    ...
    const handleAddBook = (shelfId: string) => {
        ...
        onAddBook?.(shelfId, s?.name);
    };
    const handleEditShelf = (shelfId: string) =>
        window.dispatchEvent(
            new CustomEvent("editShelf", { detail: { shelfId } }),
        );
    const handleDeleteShelf = (shelfId: string) =>
        window.dispatchEvent(
            new CustomEvent("deleteShelf", { detail: { shelfId } }),
        );
        ...
        >
            ...
            >
                ...
                <div
                    style={{
                        ...
                        boxShadow:
                            "inset 0 2px 0 rgba(255,255,255,0.2), inset 0 -3px 6px rgba(0,0,0,0.25)",
                ...
                >
                    ...
                    >
                        <SortableContext
                            items={shelvesToRender.map((s) => s.id)}
                        ...
                        >
                            {shelvesToRender.map((shelf, idx) => (
                                <SortableShelf
                                    ...
                                    shelf={shelf}
                                    books={
                                        booksDistribution.get(shelf.id) || []
                                    }
                ...
                <div
                    style={{
                        ...
                        boxShadow:
                            "inset 0 3px 5px rgba(0,0,0,0.25), 0 5px 18px rgba(0,0,0,0.45)",
            ...
            <div
                style={{
                    ...
                    height: 14,
                    background:
                        "linear-gradient(180deg, #b89060 0%, #9a7440 100%)",
