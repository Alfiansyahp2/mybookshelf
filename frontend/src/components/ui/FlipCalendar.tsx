...
export default function FlipCalendar({ onClick }: { onClick?: () => void }) {
    ...
    return (
        <div
            ...
            style={{
                transform: onClick ? "scale(0.85)" : "scale(0.85)",
                transformOrigin: "bottom",
            }}
        >
            {/* Base */}
            <div
                className="absolute bottom-0 w-full h-3 bg-[#2a1a10] rounded-sm shadow-md"
...
function FlipCard({
    ...
    return (
        <div className="relative flex flex-col items-center">
            {/* Ring */}
            <div
                className="absolute -top-3 w-3 h-4 border-2 border-gray-300 rounded-full z-20"
                style={{ clipPath: "polygon(0 0, 100% 0, 100% 70%, 0 70%)" }}
            />
            <div
                className="absolute -top-3 w-3 h-4 border-2 border-gray-400 rounded-full z-0"
                style={{
            ...
            <div className="relative w-8 h-11 bg-[#f0eee4] rounded shadow-sm flex items-center justify-center border border-[#dcd8c6] overflow-hidden mt-1">
                ...
                {isText ? (
                    <div
                        className="flex flex-col items-center justify-center leading-[0.85] font-bold text-[#1a1a1a] text-[10px]"
