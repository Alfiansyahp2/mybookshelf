...
const TIMEZONES = [
    {
        id: "local",
        labelKey: "time_settings.local_time",
        fallback: "Waktu Lokal (Perangkat)",
        tz: "",
    },
    {
        id: "id-wib",
        labelKey: "time_settings.indonesia",
        fallback: "Indonesia (WIB)",
        tz: "Asia/Jakarta",
    },
    {
        id: "jp",
        labelKey: "time_settings.japan",
        fallback: "Jepang (JST)",
        tz: "Asia/Tokyo",
    },
    {
        id: "kr",
        labelKey: "time_settings.korea",
        fallback: "Korea Selatan (KST)",
        tz: "Asia/Seoul",
    },
    {
        id: "uk",
        labelKey: "time_settings.uk",
        fallback: "Inggris (GMT/BST)",
        tz: "Europe/London",
    },
    {
        id: "us-ny",
        labelKey: "time_settings.usa",
        fallback: "Amerika Serikat (EST/EDT)",
        tz: "America/New_York",
    },
    {
        id: "au-syd",
        labelKey: "time_settings.australia",
        fallback: "Australia (AEST/AEDT)",
        tz: "Australia/Sydney",
    },
];
...
export default function BigDigitalClock() {
    ...
    const getDisplayTime = () => {
        ...
        try {
            ...
            const tzString = time.toLocaleString("en-US", {
                timeZone: tzData.tz,
            });
        ...
        <>
            <div
                ...
                style={{
                    ...
                    backgroundImage:
                        "repeating-linear-gradient(0deg, transparent, transparent 1px, rgba(0,0,0,0.15) 1px, rgba(0,0,0,0.15) 3px)",
                    boxShadow:
                        "inset 0 2px 4px rgba(0,0,0,0.4), 0 2px 4px -1px rgba(0,0,0,0.2)",
                }}
            >
                <div
                    ...
                    style={{
                        textShadow:
                            "0 0 5px rgba(255,160,60,0.7), 0 0 12px rgba(255,120,30,0.4)",
                        fontVariantNumeric: "tabular-nums",
                    }}
                >
                    <span>
                        {hh}
                        <span style={{ opacity: isBlinking ? 1 : 0.3 }}>:</span>
                        {mm}
                    </span>
                    <div className="flex flex-col items-center justify-center -mt-0.5">
                        <span className="text-[10px] sm:text-[11px] tracking-normal text-[#ffdbb0]">
                            26°C
                        </span>
                        {format === "12h" && (
                            <span className="text-[8px] leading-none tracking-normal text-[#ffdbb0] mt-0.5 opacity-80">
                                {ampm}
                            </span>
                        )}
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {TIMEZONES.map((tz) => (
                                <label
                                    key={tz.id}
                                    className="flex items-center gap-3 p-3 rounded-xl border border-walnut/10 hover:bg-cream/40 cursor-pointer transition-colors"
                                >
                                    <input
                                        type="radio"
                                        name="clockTimezone"
                                    ...
                                    />
                                    <div className="font-medium text-darkBrown text-sm">
                                        {t(tz.labelKey, tz.fallback)}
                                    </div>
                                </label>
                            ))}
                        </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            <label className="flex items-center gap-3 p-3 rounded-xl border border-walnut/10 hover:bg-cream/40 cursor-pointer transition-colors">
                                ...
                                />
                                <div>
                                    <div className="font-semibold text-darkBrown text-sm">
                                        {t(
                                            "time_settings.format_24",
                                            "Format 24 Jam",
                                        )}
                                    </div>
                                    <div className="text-xs text-walnut/60">
                                        {t(
                                            "time_settings.format_24_example",
                                            "Contoh: 14:30",
                                        )}
                                    </div>
                                </div>
                            ...
                            <label className="flex items-center gap-3 p-3 rounded-xl border border-walnut/10 hover:bg-cream/40 cursor-pointer transition-colors">
                                ...
                                <div>
                                    <div className="font-semibold text-darkBrown text-sm">
                                        {t(
                                            "time_settings.format_12",
                                            "Format 12 Jam",
                                        )}
                                    </div>
                                    <div className="text-xs text-walnut/60">
                                        {t(
                                            "time_settings.format_12_example",
                                            "Contoh: 02:30 PM",
                                        )}
                                    </div>
                                </div>
