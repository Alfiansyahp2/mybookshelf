...
export default function NotificationCenter() {
    ...
    const formatDate = (isoStr: string) => {
        ...
        if (diffMins < 60)
            return t("notification.mins_ago", "{{count}} mnt lalu", {
                count: diffMins,
            });
        if (diffHours < 24)
            return t("notification.hours_ago", "{{count}} jam lalu", {
                count: diffHours,
            });
                ...
                {isOpen && (
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between p-4 border-b border-walnut/10 bg-white/50">
                            <h3 className="font-serif font-bold text-darkBrown text-lg">
                                {t("notification.title", "Notifikasi")}
                            </h3>
                            <div className="flex gap-2">
                                {unreadCount > 0 && (
                                    <button
                                        onClick={markAllAsRead}
                                        className="p-1.5 rounded-md hover:bg-walnut/10 text-walnut transition-colors"
                                        title={t(
                                            ...
                                            <motion.div
                                                ...
                                                className={`group relative flex gap-3 p-3 rounded-xl transition-all ${
                                                    notif.read
                                                        ...
                                                        : "bg-white shadow-sm border border-walnut/5"
                                                }`}
                                                onClick={() =>
                                                    !notif.read &&
                                                    markAsRead(notif.id)
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex justify-between items-start mb-0.5">
                                                        <h4
                                                            className={`text-sm font-bold truncate pr-4 ${notif.read ? "text-darkBrown/70" : "text-darkBrown"}`}
                                                        >
                                                            {notif.title}
                                                        </h4>
                                                    ...
                                                    <p
                                                        className={`text-xs leading-relaxed ${notif.read ? "text-walnut/60" : "text-walnut/90"}`}
                                                    >
                                                        {notif.message}
                                                    </p>
