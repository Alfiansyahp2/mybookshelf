    ...
    >
      <div className="bg-white rounded-2xl p-6 border border-walnut/10 shadow-sm">
        <h2 className="text-xl font-serif font-semibold text-darkBrown mb-6 flex items-center gap-2">
          ...
          {t('settings.data.data_management', 'Data Management')}
        </h2>
        <div className="space-y-3">
          ...
          >
            ...
            <span className="font-medium">{t('settings.data.export_data', 'Export All Data')}</span>
          </button>

          <div className="bg-cream/30 rounded-xl p-4">
            <p className="text-sm text-walnut/80 mb-2">
              {t('settings.data.export_desc', 'This will download all your books, settings, and reading progress as a JSON file that you can import later.')}
            </p>
            <div className="flex items-center gap-2 text-xs text-walnut/60">
              <Database className="w-4 h-4" />
              <span>{t('settings.data.file_size', 'File size: ~50KB')}</span>
            </div>
          </div>
          ...
          >
            ...
            <span className="font-medium">{t('settings.data.clear_data', 'Clear All Data')}</span>
          </button>

          <div className="bg-red-50 rounded-xl p-4 border-2 border-red-200">
            <p className="text-sm text-red-800">
              <strong>{t('settings.data.warning', 'Warning')}:</strong> {t('settings.data.warning_desc', 'This will permanently delete all your books, settings, and reading progress. This action cannot be undone.')}
            </p>
          </div>
        </div>
      </div>
