...
export default function ReadingCalendarModal({
    isOpen,
    onClose,
    books,
}: ReadingCalendarModalProps) {
                        ...
                        <div className="flex items-center justify-between md:justify-end gap-2 md:gap-6 w-full md:w-auto">
                            ...
                            <div className="flex items-center gap-4">
                                <button
                                    ...
                                    className="p-1 hover:bg-[#8B7355]/10 rounded-full text-[#8B7355]"
                                >
                                    <ChevronLeft size={20} />
                                </button>
                                <button
                        ...
                        <div className="grid grid-cols-7 border-b border-[#8B7355]/20 bg-[#fdfbf7]">
                            {Array.from({ length: 7 }).map((_, idx) => {
                                ...
                                return (
                                    <div
                                        key={idx}
                                        className="text-center py-2 text-[11px] font-bold text-[#8B7355] tracking-wider uppercase"
                                    >
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex justify-center mb-1">
                                            <div
                                                className={`w-7 h-7 flex items-center justify-center rounded-full text-xs font-bold ${isToday ? "bg-[#8B7355] text-white shadow-sm" : "text-[#5C4532]"}`}
                                            >
                                                {cell.date.getDate() === 1
                                                    ? `${cell.date.toLocaleDateString(t("locale", "id-ID"), { month: "short" })} ${cell.date.getDate()}`
                                                    : cell.date.getDate()}
                                            </div>
                                        </div>
                            ...
                            <>
                                ...
                                <motion.div
                                    ...
                                    style={{
                                        ...
                                        top: Math.min(
                                            selectedBook.rect.bottom + 10,
                                            window.innerHeight - 300,
                                        ),
                                        left: Math.min(
                                            selectedBook.rect.left,
                                    ...
                                    <div className="p-6 pt-12">
                                        ...
                                        <div className="flex items-start gap-3 text-sm">
                                            <div className="pt-0.5">
                                                <BookOpen
                                                    size={18}
                                                    className="text-[#8B7355]/60"
                                                />
                                            </div>
                                            <div className="text-[#2a1a08]">
                                                <div className="font-medium">
                                                    {t(
                                                        "modals.readingCalendar.finished_reading",
                                                        "Finished reading",
                                                    )}
                                                </div>
                                                <div className="text-[#8B7355]/80">
                                                    {selectedBook.book.pages}{" "}
                                                    {t(
                                                        "modals.readingCalendar.pages",
                                                        "pages",
                                                    )}{" "}
                                                    • {selectedBook.book.genre}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
