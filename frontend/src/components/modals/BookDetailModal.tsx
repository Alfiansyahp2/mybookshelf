...
export default function BookDetailModal({
    ...
    const [showMarkAsReadDatePicker, setShowMarkAsReadDatePicker] =
        useState(false);
    ...
    const tabs: { id: RightTab; label: string; icon: React.ReactNode }[] = [
        {
            id: "progress",
            label: t("bookDetail.tabs.progress", "Progress"),
            icon: <BookOpen className="w-3.5 h-3.5" />,
        },
        {
            id: "session",
            label: t("bookDetail.tabs.session", "Sesi"),
            icon: <Clock className="w-3.5 h-3.5" />,
        },
        {
            id: "notes",
            label: t("bookDetail.tabs.notes", "Catatan"),
            icon: <Edit className="w-3.5 h-3.5" />,
        },
        {
            id: "info",
            label: t("bookDetail.tabs.info", "Info"),
    ...
    const handleRating = (r: number) => {
        setUserRating(r);
        updateRating.mutate({ id: book.id, rating: r });
    };
    const handleProgress = (p: number) =>
        updateProgress.mutate({ id: book.id, currentPage: p });
    const handleNotes = () => {
        updateNotes.mutate({ id: book.id, notes: tempNotes });
        setUserNotes(tempNotes);
        setIsEditingNotes(false);
    };
    const handleStart = () => {
        if (book.status === "unread") startReading.mutate(book.id);
    };
    const handleFinish = () => {
