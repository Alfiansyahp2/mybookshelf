...
export default function EditBookModal({
    ...
    const handleCoverSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > 5 * 1024 * 1024) {
            alert(
                t(
                    "modals.editBook.file_too_large",
        ...
        uploadCover.mutate(
            { id: book.id, file },
            {
                ...
                onError: (err: any) => {
                    ...
                    alert(
                        t(
                            "modals.editBook.upload_error",
                            "Gagal mengupload cover. Pesan dari server: {{msg}}",
                            { msg: backendMsg },
                        ),
                    );
        ...
        updateBook.mutate(
            {
                ...
                updates: {
                    ...
                    borrowedDate:
                        formData.status === "borrowed"
                            ? formData.borrowedDate
                            : "",
                    dueDate:
                        formData.status === "borrowed" && formData.dueDate
                            ? formData.dueDate
                            : "",
                },
            },
    ...
    const handleDelete = () => {
        if (
            ...
            window.confirm(
                t(
                    "modals.editBook.confirm_delete",
            ...
            <form onSubmit={handleSubmit} className="flex gap-6">
                ...
                <div className="w-1/3 flex-shrink-0">
                    ...
                    >
                        ...
                        {!coverPreview && (
                            <div className="h-full p-6 flex flex-col justify-between">
                                <div>
                                    <h3
                                        className="text-white font-serif font-bold text-xl mb-2 leading-tight"
                                        style={{
                                            textShadow:
                                ...
                                {isUploading ? (
                                    ...
                                    <div className="flex flex-col items-center gap-2">
                                        ...
                                        <span className="text-xs font-medium">
                                            {coverPreview
                                                ? t(
                                                      "modals.editBook.change_cover",
                                                      "Ganti Cover",
                                                  )
                                                : t(
                                                      "modals.editBook.upload_cover",
                                                      "Upload Cover",
                                                  )}
                                        </span>
                                    </div>
                    ...
                    <p className="text-center text-xs text-walnut/40 mt-2">
                        {t(
                            "modals.editBook.upload_hint",
                            "Klik cover untuk {{action}} foto",
                            {
                                action: coverPreview
                                    ? t("modals.editBook.change", "mengganti")
                                    : t("modals.editBook.upload", "upload"),
                            },
                        )}
                    </p>
                    ...
                    <div className="mt-4 space-y-3">
                        <div className="text-center">
                            <div className="text-3xl font-bold text-darkBrown">
                                {formData.pages || book.pages || 0}
                            </div>
                            <div className="text-xs text-walnut/70">
                                {t("modals.editBook.pages", "Pages")}
                            </div>
                        </div>
                ...
                <div className="flex-1 space-y-4">
                    ...
                    <div className="flex items-center gap-3 pb-4 border-b border-walnut/10">
                        ...
                        <h2 className="text-2xl font-serif font-bold text-darkBrown">
                            {t(
                                "modals.editBook.edit_info",
