...
export default function Achievements() {
    ...
    const stats = useMemo(() => {
        ...
        const pages = finished.reduce(
            (sum: number, b: any) => sum + (b.pages || b.totalPages || 0),
            0,
        );
        const genres = new Set(
            books.filter((b: any) => b.genre).map((b: any) => b.genre),
        );
    ...
    const achievementsList = useMemo(() => {
        return apiAchievements.map((ach) => {
            let progress = 0;
            if (ach.category === "reading" && ach.title.includes("Marathon"))
                progress = stats.pages;
            else if (ach.category === "reading") progress = stats.finished;
            else if (
                ach.category === "collections" &&
                ach.title.includes("Rak")
            )
                progress = stats.shelves;
            else if (ach.category === "collections") progress = stats.total;
            else if (ach.title.includes("Genre")) progress = stats.genres;
            else if (
                ach.title.includes("Bintang") ||
                ach.title.includes("Sempurna")
            )
                progress = stats.fiveStar;
            else if (
                ach.title.includes("Favorit") ||
                ach.title.includes("Setia")
            )
                progress = stats.favs;
            else if (
                ach.title.includes("Wishlist") ||
                ach.title.includes("Pemimpi")
            )
                progress = stats.wishlist;
            ...
            return {
                id: ach.id,
                title: t(
                    `achievements.data.${ach.title.replace(/\\./g, "_")}.title`,
                    ach.title,
                ),
                description: t(
                    `achievements.data.${ach.description.replace(/\\./g, "_")}`,
                    ach.description,
                ),
                icon: iconMap[ach.icon] || Trophy,
                category:
                    ach.category === "special"
                        ? "special"
                        : ach.category === "collections"
                          ? "collection"
                          : "milestone",
                    ...
                    return (
                        ...
                        >
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                >
                                    {t(
                                        `achievements.items.${ach.id}.title`,
                                        ach.title,
                                    )}
                                </h3>
                                <p
                                ...
                                >
                                    {t(
                                        `achievements.items.${ach.id}.description`,
                                        ach.description,
                                    )}
                                </p>
